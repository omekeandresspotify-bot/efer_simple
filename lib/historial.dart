...
class FichaFabricacionPage extends StatelessWidget {
  ...
  List<Map<String, dynamic>> obtenerResumenVidrios() {
    final Map<String, Map<String, dynamic>> agrupados = {};

    for (final resultado in resultados) {
      final esTermo = resultado.producto.toLowerCase().contains('termo');

      final tipoVidrio = esTermo ? 'Termopanel 4 + 9,5 + 4' : 'Vidrio 4 mm';

      for (final corte in resultado.cortes) {
        if (corte.pieza != 'CRISTAL') {
          continue;
        }
        ...
        final ancho = corte.ancho!.round();
        final alto = corte.alto!.round();

        final clave = '$tipoVidrio|$ancho|$alto';

        if (agrupados.containsKey(clave)) {
          agrupados[clave]!['cantidad'] += corte.cantidad;
        } else {
          agrupados[clave] = {
            'tipo': tipoVidrio,
            'ancho': ancho,
  ...
  Future<void> imprimirFicha(BuildContext context) async {
    try {
      ...
      // ==========================================================
      // SEGUNDA HOJA - RESUMEN DE VIDRIOS
      // ==========================================================

      final resumenVidrios = obtenerResumenVidrios();
            ...
            return [
              ...
              pw.Table(
                ...
                children: [
                  pw.TableRow(
                    children: [
                      celdaPdf('Nº', negrita: true, centrado: true),

                      celdaPdf('TIPO DE VIDRIO', negrita: true),

                      celdaPdf('ANCHO', negrita: true, centrado: true),

                      celdaPdf('ALTO', negrita: true, centrado: true),

                      celdaPdf('CANT.', negrita: true, centrado: true),
                    ],
                  ),

                  ...List.generate(resumenVidrios.length, (index) {
                    final vidrio = resumenVidrios[index];

                    return pw.TableRow(
                      children: [
                        celdaPdf('${index + 1}', centrado: true),

                        celdaPdf(vidrio['tipo'] as String),

                        celdaPdf('${vidrio['ancho']} mm', centrado: true),

                        celdaPdf('${vidrio['alto']} mm', centrado: true),
              ...
              pw.Container(
                ...
                child: pw.Text(
                  'Las medidas de los vidrios corresponden '
                  'directamente a la Pauta de Fabricación.',
                  style: const pw.TextStyle(fontSize: 8),
                ),
              ),
            ];
